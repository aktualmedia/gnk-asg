...
Already-published share URLs remain preserved so previously shared LinkedIn posts do not break.
"""
...
def make_image(row: dict, target: Path) -> None:
    ...
    y = 219
    for line in textwrap.wrap(title, width=39)[:3]:
...
def main() -> None:
    ...
    for row in rows[:MAX_PREVIEWS]:
        ...
        if not uid or uid in generated:
            continue
        generated.add(uid)
    ...
    print(f'Generated or refreshed {len(generated)} individual social previews for public news.')
