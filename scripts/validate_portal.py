...
def check_structure(seo=False):
 ...
 for name in ['group-globe-3d.js','network-motion.js','portal-layout.js']:
  ok('Aplikacija učitava: '+name) if name in app else fail('Aplikacija ne učitava: '+name)
 if 'group-google-map.js' not in app and 'group-location-weather.js' not in app and 'location-recovery.js' not in app: ok('Aplikacija ne učitava uklonjene prazne panele karte i vremena')
 else: fail('Aplikacija još učitava uklonjene prazne panele karte ili vremena')
 ...
 if "cron: '19 * * * *'" in hourly and 'data/market.json' not in hourly and 'update_feeds_v2.py' in hourly and 'data/update_status.json' in hourly: ok('Satni workflow za vijesti izoliran je od tržišnih podataka i izvršava se u 19. minuti')
 else: fail('Satni workflow za vijesti nije pravilno izoliran ili raspored nije usklađen')
