#!/usr/bin/env python3
"""Validate GNK ASG public portal structure, SEO, 3D network and refreshed datasets."""
...
SOCIAL_IMAGE = SITE + "assets/gnk-asg-social-card.svg"
ERRORS: list[str] = []
PASSED: list[str] = []
PUBLIC_PAGES = [
    ("index.html", SITE),
    ("en/index.html", SITE + "en/"),
    ("sadrzaj/index.html", SITE + "sadrzaj/"),
    ("financije/index.html", SITE + "financije/"),
    ("tehnologija/index.html", SITE + "tehnologija/"),
    ("intelligence-desk/index.html", SITE + "intelligence-desk/"),
    ("registri/index.html", SITE + "registri/"),
    ("instalacija/index.html", SITE + "instalacija/"),
]
...
def check_structure() -> None:
    required = [
        ...
        "assets/group-globe-3d.js", "assets/group-globe-3d.css", "assets/gnk-asg-social-card.svg", "assets/admin-status-only.js",
    ...
    if 'name="robots" content="noindex,nofollow,noarchive"' in admin:
        ok("Admin stranica isključena je iz indeksiranja")
    else:
        fail("Admin stranica nema očekivani noindex robots status")
...
def check_seo() -> None:
    required_tokens = (
        ...
        'property="og:site_name"', 'property="og:image"', 'name="twitter:card"',
        'name="twitter:title"', 'name="twitter:description"', 'name="twitter:image"',
        'type="application/ld+json"', 'SEO:BEGIN generated by scripts/generate_seo.py',
    )
    for relative_path, canonical in PUBLIC_PAGES:
        html = (ROOT / relative_path).read_text(encoding="utf-8")
        missing = [token for token in required_tokens if token not in html]
        if missing:
            fail(f"SEO paket nije potpun za {relative_path}: {', '.join(missing)}")
            continue
        if f'href="{canonical}"' not in html or f'content="{canonical}"' not in html:
            fail(f"Canonical/OG URL nije usklađen za {relative_path}")
            continue
        if SOCIAL_IMAGE not in html:
            fail(f"Social preview slika nije povezana za {relative_path}")
            continue
        ok(f"Meta, canonical, Open Graph, Twitter i JSON-LD: {relative_path}")
    sitemap = (ROOT / "sitemap.xml").read_text(encoding="utf-8")
    missing_urls = [canonical for _, canonical in PUBLIC_PAGES if canonical not in sitemap]
    if missing_urls:
        fail("Sitemap ne sadrži sve javne URL-ove: " + ", ".join(missing_urls))
    else:
        ok("Sitemap sadrži svih osam javnih URL-ova")
    robots = (ROOT / "robots.txt").read_text(encoding="utf-8")
    if "Disallow: /gnk-asg/admin/" in robots and "Sitemap: " + SITE + "sitemap.xml" in robots:
        ok("Robots.txt zadržava admin izvan indeksa i navodi sitemap")
    else:
        fail("Robots.txt nema očekivanu admin/sitemap politiku")
...
def main() -> int:
    ...
    parser.add_argument("--seo", action="store_true", help="Validate generated metadata for all public pages")
