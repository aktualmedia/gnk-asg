...
PAGE_TERMS = {
 ...
 'sadrzaj/': 'sadržaj portala, korporativni podatci, financije, tehnologija, tržišta, Intelligence Desk, poslovne vijesti, javni dokumenti',
...
PAGES = [
 ...
 {'path':'sadrzaj/','file':'sadrzaj/index.html','lang':'hr','locale':'hr_HR','changefreq':'weekly','priority':'0.8','title':'Sadržaj portala | GNK ASG d.o.o. | GNK DINAMO Ltd. | Nermin Sefić','description':'Pregled javnih sadržaja o subjektima GNK ASG d.o.o., GNK DINAMO Ltd. i Nermin Sefić: financije, mreža društava, tehnologija, Intelligence Desk, registri, tržišta i vijesti.','type':'CollectionPage','name':'Sadržaj portala GNK ASG d.o.o. i GNK DINAMO Ltd.','crumb':'Sadržaj'},
...
def keywords(p): return (CORE_EN if p['lang'] == 'en' else CORE_HR) + ', ' + PAGE_TERMS.get(p['path'], '')
def entities():
 return [
...
def metadata(p):
 u = url(p); other = 'en_US' if p['locale']=='hr_HR' else 'hr_HR'; keys = keywords(p)
