(() => {
  ...
  const tr = () => en() ? {
    ...
    existing:'Existing group company', planned:'Planned expansion 2026', hq:'Central headquarters', finance:'Displayed financial context',
    ...
    existing:'Postojeće društvo grupe', planned:'Planirano širenje 2026.', hq:'Središnje sjedište', finance:'Prikazani financijski kontekst',
  ...
  function insightHtml(item, fact) {
    ...
    return `<div class="insight-heading"><small>${esc(T.select)}</small><strong>${esc(nName(item))}</strong><span>${esc(nPlace(item))}</span></div>` +
      ...
      `<div class="wide"><label>${esc(T.status)}</label><b>${esc(positionState(item))}</b></div></div>` +
