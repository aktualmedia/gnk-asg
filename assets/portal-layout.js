(() => {
  ...
  const copy = () => isEn() ? {
    title:'Downloads and snapshots', note:'Map, demographics and weather follow the selected point.', staticPdf:'Static PDF', mapPdf:'2D PDF', globePdf:'3D PDF', mapPng:'2D Image', globePng:'3D Image', busy:'Preparing…', missing:'Unavailable'
  } : {
    title:'Preuzimanja i prikazi', note:'Karta, demografija i vrijeme prate odabranu točku.', staticPdf:'Statični PDF', mapPdf:'2D PDF', globePdf:'3D PDF', mapPng:'2D slika', globePng:'3D slika', busy:'Pripremam…', missing:'Nije dostupno'
  };
  ...
  function arrange() {
    ...
    const map = $('googleLocationMap'), facts = shell.querySelector('.network-sidebar'), weather = $('locationWeatherPanel'), overview = $('networkOverviewVisual');
    if (map && dock.firstElementChild !== map) dock.prepend(map);
  ...
  function init() {
    ...
    const timer = setInterval(() => { attempts += 1; stable = arrange() ? stable + 1 : 0; if (stable >= 35 || attempts >= 190) clearInterval(timer); }, 110);
