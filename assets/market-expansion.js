(() => {
  ...
  function render() {
    ...
    target.innerHTML = `<div class="market-expansion-head"><div><p class="eyebrow">${c.eyebrow}</p><h3>${c.title}</h3></div><p>${c.intro}</p></div><div class="exchange-layout"><article class="exchange-board"><div class="exchange-board-title"><strong>${c.exchange}</strong><span class="exchange-refresh">${c.checked}: ${c.snapshot}</span></div><div class="exchange-grid">${stocks.markets.map(marketCard).join('')}</div></article><aside class="asg-panel"><span class="asg-status">${c.status}</span><div class="asg-coin-wrap"><img class="asg-coin-art" src="assets/asg-gold-coin.svg" alt="ASG Gold Reference Asset"><div><h4>ASG Stable Coin</h4><span class="issuer">${c.issuer}</span></div></div><div class="asg-value"><small>${c.value}</small><strong>${assetValue} / ASG</strong>${asg.change_30d_percent == null ? '' : change(asg.change_30d_percent)}</div><div class="asg-facts"><div class="asg-fact"><small>${c.supply}</small><strong>${number(asg.total_supply)} ASG</strong></div><div class="asg-fact"><small>${c.rule}</small><strong>${isEnglish() ? asg.reference_rule_en : asg.reference_rule_hr}</strong></div></div><p class="asg-legal">${c.legal}</p></aside></div><p class="market-disclosure">${c.disclosure}</p>`;
    injectAsgCard();
  }
  async function load() {
    try {
