(() => {
  ...
  const t = () => isEnglish() ? {
    eyebrow:'Markets and Reference Assets', title:'Stock Exchange Monitor & ASG Gold Reference', intro:'Official exchange links and public market indicators alongside an informational gold-linked ASG profile.', exchange:'Stock Exchange Monitor', updated:'Updated', croatia:'Croatia', world:'Global market', verify:'Verify at official source →', awaiting:'Open official online display', issuer:'Issuer reference: GNK DINAMO Ltd.', status:'Pre-launch informational display', value:'Indicative ASG reference value', supply:'Reference supply', rule:'Reference rule', change:'Gold-linked 30d change', legal:'Informational gold-linked value concept. Not a public offer, prospectus, crypto-asset white paper, investment advice or confirmation of issuance of a regulated stablecoin.', disclosure:'Stock-market and reference-asset data are informational. Values may be delayed and must be verified at the respective official source.'
  } : {
    eyebrow:'Tržišta i referentna imovina', title:'Burzovni monitor i ASG Gold Reference', intro:'Službene burzovne poveznice i javni tržišni pokazatelji uz informativni ASG profil vezan uz vrijednost zlata.', exchange:'Burzovni monitor', updated:'Ažurirano', croatia:'Hrvatska', world:'Globalno tržište', verify:'Provjeri službeni izvor →', awaiting:'Otvori službeni online prikaz', issuer:'Referentni izdavatelj: GNK DINAMO Ltd.', status:'Pre-launch informativni prikaz', value:'Indikativna referentna vrijednost ASG', supply:'Predviđena količina', rule:'Referentno pravilo', change:'Promjena prema zlatu / 30 d', legal:'Informativni koncept vrijednosti vezan uz zlato. Nije javna ponuda, prospekt, bijela knjiga kriptoimovine, investicijski savjet niti potvrda izdavanja reguliranog stablecoina.', disclosure:'Burzovni i referentni tržišni podatci prikazuju se informativno. Vrijednosti mogu biti odgođene te se provjeravaju na službenom izvoru.'
  };
  const money = (value, code) => value == null ? '—' : new Intl.NumberFormat(isEnglish() ? 'en-US' : 'hr-HR', {style:'currency', currency:code, maximumFractionDigits:2}).format(Number(value));
  ...
  function marketCard(item) {
    ...
    const delay = isEnglish() ? item.delay_en : item.delay_hr;
  ...
  function render() {
    ...
    const c = t();
    const updated = stocks.updated_at ? new Date(stocks.updated_at).toLocaleString(isEnglish() ? 'en-GB' : 'hr-HR') : '—';
