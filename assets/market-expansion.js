(() => {
  ...
  const t = () => isEnglish() ? {
    eyebrow:'Markets and Reference Assets', title:'Stock Exchange Monitor & ASG Gold Reference', intro:'Official exchange links and public market indicators alongside an informational gold-linked ASG profile.', exchange:'Stock Exchange Monitor', checked:'Source status', snapshot:'SNAPSHOT', sourceCheck:'Source check', croatia:'Croatia', world:'Global market', verify:'Verify at official source →', awaiting:'Official source link', issuer:'Issuer reference: GNK DINAMO Ltd.', status:'Pre-launch informational display', value:'Indicative ASG reference value', supply:'Proposed reference supply', rule:'Reference rule', legal:'Informational gold-linked value concept. Not a public offer, prospectus, crypto-asset white paper, investment advice or confirmation of issuance of a regulated stablecoin.', disclosure:'Stock-market and reference-asset data are informational. Values may be delayed and must be verified at the respective official source.', cardNote:'Gold reference · pre-launch', retained:'Official source verification required'
  } : {
    eyebrow:'Tržišta i referentna imovina', title:'Burzovni monitor i ASG Gold Reference', intro:'Službene burzovne poveznice i javni tržišni pokazatelji uz informativni ASG profil vezan uz vrijednost zlata.', exchange:'Burzovni monitor', checked:'Status izvora', snapshot:'SNAPSHOT', sourceCheck:'Provjera izvora', croatia:'Hrvatska', world:'Globalno tržište', verify:'Provjeri službeni izvor →', awaiting:'Službeni izvor', issuer:'Referentni izdavatelj: GNK DINAMO Ltd.', status:'Pre-launch informativni prikaz', value:'Indikativna referentna vrijednost ASG', supply:'Predviđena referentna količina', rule:'Referentno pravilo', legal:'Informativni koncept vrijednosti vezan uz zlato. Nije javna ponuda, prospekt, bijela knjiga kriptoimovine, investicijski savjet niti potvrda izdavanja reguliranog stablecoina.', disclosure:'Burzovni i referentni tržišni podatci prikazuju se informativno. Vrijednosti mogu biti odgođene te se provjeravaju na službenom izvoru.', cardNote:'Referenca na zlato · pre-launch', retained:'Potrebna provjera službenog izvora'
  };
  ...
  function formatDate(value) {
    if (!value) return t().snapshot;
    const d = new Date(value);
    if (Number.isNaN(d.getTime())) return t().snapshot;
    return `${t().sourceCheck}: ${d.toLocaleString(isEnglish() ? 'en-GB' : 'hr-HR')}`;
  }
  ...
  function marketCard(item) {
    ...
    const retained = item.status === 'public_market_feed_stale' || item.value == null;
  ...
  function render() {
    ...
    const sourceStatus = `${c.checked}: ${formatDate(stocks.updated_at || stocks.checked_at)}`;
    target.innerHTML = `<div class="market-expansion-head"><div><p class="eyebrow">${c.eyebrow}</p><h3>${c.title}</h3></div><p>${c.intro}</p></div><div class="exchange-layout"><article class="exchange-board"><div class="exchange-board-title"><strong>${c.exchange}</strong><span class="exchange-refresh">${sourceStatus}</span></div><div class="exchange-grid">${(stocks.markets || []).map(marketCard).join('')}</div></article><aside class="asg-panel"><span class="asg-status">${c.status}</span><div class="asg-coin-wrap"><img class="asg-coin-art" src="assets/asg-gold-coin.svg" alt="ASG Gold Reference Asset"><div><h4>ASG Stable Coin</h4><span class="issuer">${c.issuer}</span></div></div><div class="asg-value"><small>${c.value}</small><strong>${assetValue} / ASG</strong>${asg.change_30d_percent == null ? '' : change(asg.change_30d_percent)}</div><div class="asg-facts"><div class="asg-fact"><small>${c.supply}</small><strong>${number(asg.total_supply)} ASG</strong></div><div class="asg-fact"><small>${c.rule}</small><strong>${isEnglish() ? asg.reference_rule_en : asg.reference_rule_hr}</strong></div></div><p class="asg-legal">${c.legal}</p></aside></div><p class="market-disclosure">${c.disclosure}</p>`;
    injectAsgCard();
  }
  async function load() {
    try {
