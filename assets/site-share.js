(() => {
  ...
  const CARDS = [
    ...
    { id:'grupa', hr:'Globalna mreža', en:'Global network', path:'/podijeli/grupa/' },
    { id:'trzista', hr:'Tržišta', en:'Markets', path:'/podijeli/trzista/' },
    { id:'bpp', hr:'BPP.IS', en:'BPP.IS', path:'/podijeli/bpp/' },
  ...
  if (!document.querySelector('link[data-gnk-share-style]')) {
    ...
    css.href = ROOT + 'assets/site-share.css?v=20260527-share-picker05';
  ...
  function readState() {
    try { const value = JSON.parse(localStorage.getItem(STATE_KEY) || 'null'); return value && Number.isFinite(value.events) ? value : null; }
  ...
  function startPageVisit() {
    ...
    if (!previous) { saveState({ events:0, initializedAt:Date.now(), lastAction:'initial-view' }); return; }
    previous.events += 1; previous.lastAction = 'open-or-refresh'; previous.updatedAt = Date.now(); saveState(previous);
  }
  function countInteraction() {
    const state = readState() || { events:0, initializedAt:Date.now() };
    state.events += 1; state.lastAction = 'click'; state.updatedAt = Date.now(); saveState(state); refreshVisits();
  }
  function timedVisits(now = new Date()) {
    ...
    while (cursor < now) {
      const hourEnd = new Date(cursor.getTime() + 3600000), segmentEnd = hourEnd < now ? hourEnd : now;
      value += hourlyRate(localHour(cursor)) * ((segmentEnd.getTime() - cursor.getTime()) / 3600000); cursor = segmentEnd;
    }
    return Math.floor(value);
  }
  function indicativeVisits(now = new Date()) { const state = readState(); return timedVisits(now) + (state ? state.events : 0); }
  function cardUrl(card) { const url = new URL(card.path, location.origin); url.search = PREVIEW; return url.href; }
  function activeCard() {
