(() => {
  ...
  function render() {
    ...
    if (updated && dataset.updated_at) updated.textContent = (en ? 'Updated: ' : 'Ažurirano: ') + new Date(dataset.updated_at).toLocaleString(en ? 'en-GB' : 'hr-HR');
    const ticker = $('#ticker');
    if (ticker) {
      ...
      ticker.innerHTML = tape + '<span><b>GNK ASG</b> DIGITAL ASSETS MONITOR · ' + (en ? 'INFORMATIONAL DISPLAY' : 'INFORMATIVNI PRIKAZ') + '</span>' + tape;
    }
    convert();
  }
  async function init() {
