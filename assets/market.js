(() => {
  ...
  function money(value, code) {
    ...
    return new Intl.NumberFormat('hr-HR', { style:'currency', currency: code.toUpperCase(), maximumFractionDigits: digits }).format(number);
  }
  ...
  function render() {
    ...
    if (!dataset.coins || !dataset.coins.length) {
      grid.innerHTML = '<article class="coin"><strong>Tržišni podatci aktiviraju se nakon prvog automatskog ažuriranja.</strong><p>Satni workflow priprema indikativne cijene digitalne imovine.</p></article>';
      return;
    }
    ...
    if (updated && dataset.updated_at) updated.textContent = 'Ažurirano: ' + new Date(dataset.updated_at).toLocaleString('hr-HR');
    const ticker = $('#ticker');
    if (ticker) {
      ...
      ticker.innerHTML = tape + '<span><b>GNK ASG</b> DIGITAL ASSETS MONITOR · INFORMATIVNI PRIKAZ</span>' + tape;
    }
    convert();
  }
  async function init() {
