(() => {
  ...
  function buildShell() {
    ...
    const c = t();
    section.innerHTML = `<div class="container"><div class="section-head"><div><p class="eyebrow">${c.eyebrow}</p><h2>${c.title}</h2></div><p>${c.text}</p></div><div class="network-shell"><div class="network-topbar"><div class="network-stats"><div class="network-stat"><strong>33</strong><span>${c.current}</span></div><div class="network-stat"><strong>+12</strong><span>${c.expansion}</span></div><div class="network-stat"><strong>45</strong><span>${c.after}</span></div></div><div class="network-filters"><button data-filter="all" class="active">${c.all}</button><button data-filter="active">${c.active}</button><button data-filter="planned">${c.planned}</button><button data-filter="outside">${c.outside}</button></div></div><div class="network-layout"><div class="network-canvas"><svg id="networkSvg" viewBox="0 0 1040 545" aria-label="${c.title}"></svg><div class="network-controls"><button type="button" data-zoom="in">+</button><button type="button" data-zoom="out">−</button><button type="button" data-zoom="reset">↺</button></div></div><aside class="network-sidebar"><h3>${c.legend}</h3><div><div class="legend-row"><i class="legend-symbol hq"></i><div><strong>${c.hq}</strong><span>${c.hqText}</span></div></div><div class="legend-row"><i class="legend-symbol active"></i><div><strong>${c.live}</strong><span>${c.liveText}</span></div></div><div class="legend-row"><i class="legend-symbol planned"></i><div><strong>${c.plan}</strong><span>${c.planText}</span></div></div></div><div class="line-legend"><div><i class="line-swatch direct"></i>${c.direct}</div><div><i class="line-swatch peer"></i>${c.peer}</div><div><i class="line-swatch planned"></i>${c.planLink}</div></div><div class="network-detail" id="networkDetail"></div><p class="network-note">${en() ? data.disclaimer_en : data.disclaimer_hr}</p></aside></div><div class="network-regions" id="networkRegions"></div></div></div>`;
