(() => {
  ...
  function render() {
    ...
    const newsFresh = Boolean(news && news.status === 'ok' && Number(news.public_items) > 0 && minutesOld(news.updated_at) <= NEWS_MAX_AGE_MINUTES);
    ...
    if (appMarketFresh) {
      const markets = marketData && marketData.digital_assets ? marketData.digital_assets.coins : 12;
      const text = en ? 'Digital Assets: ' + markets + ' assets · Live: ' + dateLabel(liveMarket.updated_at) : 'Digitalna imovina: ' + markets + ' stavki · Uživo: ' + dateLabel(liveMarket.updated_at);
      setBadge('marketBadge', text, 'ok');
    } else if (marketData && marketData.digital_assets) {
      const markets = marketData.digital_assets.coins || 0;
      const partial = !storedMarketComplete();
      const text = en ? 'Digital Assets: ' + markets + ' assets · Updated: ' + dateLabel(marketData.updated_at) + (partial ? ' · Partial refresh' : '') : 'Digitalna imovina: ' + markets + ' stavki · Ažurirano: ' + dateLabel(marketData.updated_at) + (partial ? ' · Nepotpuno osvježavanje' : '');
  ...
  async function refresh() {
    const results = await Promise.all([read('/data/update_status.json'), read('/data/fast_market_status.json')]);
    newsData = results[0];
